...
async function handleRequest(request) {
    ...
    try {
        const  PATHNAME_REGEX = "^/(cracada|wwb521)/.+$";
        //取域名第一个斜杠后的所有信息为代理链接
        ...
        //需要忽略的代理
        if (request.method == "OPTIONS" || url.length < 3 || url.indexOf('.') == -1 || url == "favicon.ico" || url == "robots.txt" || PATHNAME_REGEX && !new RegExp(PATHNAME_REGEX).test(newurl.pathname)) {
            //输出提示
            const invalid = !(request.method == "OPTIONS" || url.length === 0)
            //outBody = JSON.stringify({
                //code: invalid ? 400 : 0,
                //usage: 'Host/{URL}',
                //newurl:newurl.pathname,
                //note: '403'
            //});
