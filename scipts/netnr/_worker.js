...
async function handleRequest(request) {
    ...
    try {
		const  PATHNAME_REGEX = "^/(cracada|anaer|wwb521)/";
		//取域名第一个斜杠后的所有信息为代理链接
        ...
        if (request.method == "OPTIONS" || url.length < 3 || url.indexOf('.') == -1 || url == "favicon.ico" || url == "robots.txt" || PATHNAME_REGEX && !new RegExp(PATHNAME_REGEX).test(url.pathname)) {
            //输出提示
