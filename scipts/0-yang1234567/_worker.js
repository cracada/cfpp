...
// const CSSReplace = ["https://p.221220.xyz/https://", "https://p.221220.xyz/http://"];
const proxyHintInjection = `
//---***========================================***---提示使用代理---***========================================***---

setTimeout(() => {
  var hint = \`Warning: You are currently using a web proxy, so do not log in to any website. Click to close this hint. For further details, please visit <a href="https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/" style="color:rgb(250,250,180);">https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/</a>. <br>警告：您当前正在使用网络代理，请勿登录任何网站。单击关闭此提示。详情请见 <a href="https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/" style="color:rgb(250,250,180);">https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/</a>。\`;
...
//---***========================================***---information---***========================================***---
var now = new URL(window.__location____.href);
var base = now.host; //代理的base - proxy.com
var protocol = now.protocol; //代理的protocol
var nowlink = protocol + "//" + base + "/"; //代理前缀 https://p.221220.xyz/https://proxy.com/
var oriUrlStr = window.__location____.href.substring(nowlink.length); //如：https://p.221220.xyz/https://example.com/1?q#1
var oriUrl = new URL(oriUrlStr);

var path = now.pathname.substring(1);
//console.log("***************************----" + path);
if(!path.startsWith("http")) path = "https://p.221220.xyz/https://" + path;
...
function changeURL(relativePath){
  ...
  try {
    ...
    absolutePath = absolutePath.replace(window.__location____.href, path);
    absolutePath = absolutePath.replace(encodeURI(window.__location____.href), path);
    absolutePath = absolutePath.replace(encodeURIComponent(window.__location____.href), path);
...
class ProxyLocation {
  ...
  getOriginalHref() {
    return window.__location____.href.substring(this.getStrNPosition(window.__location____.href,"/",3)+1);
  }
  ...
  set protocol(value) {
    ...
    window.__location____.href = nowlink + oriUrl.href;
  }
...
Object.defineProperty(document, '${replaceUrlObj}', {
      get: function () {
          return new ProxyLocation(window.__location____);
      },
      set: function (url) {
          window.__location____.href = changeURL(url);
      }
});
...
function historyInject(){
  ...
  History.prototype.pushState = function (state, title, url) {
    ...
    if(url.startsWith("/" + oriUrl.href)) url = url.substring(("/" + oriUrl.href).length); // https://p.221220.xyz/https://example.com/
    if(url.startsWith("/" + oriUrl.href.substring(0, oriUrl.href.length - 1))) url = url.substring(("/" + oriUrl.href).length - 1); // https://p.221220.xyz/https://example.com (没有/在最后)
  ...
  History.prototype.replaceState = function (state, title, url) {
    ...
    //这是给duckduckgo专门的补丁，可能是window.__location____字样做了加密，导致服务器无法替换。
    //正常链接它要设置的history是/，改为proxy之后变为/https://p.221220.xyz/https://duckduckgo.com。
    ...
    if(url.startsWith("/" + oriUrl.href.replace("://", ":/"))) url = url.substring(("/" + oriUrl.href.replace("://", ":/")).length); // https://p.221220.xyz/https://example.com/
    if(url.startsWith("/" + oriUrl.href.substring(0, oriUrl.href.length - 1).replace("://", ":/"))) url = url.substring(("/" + oriUrl.href).replace("://", ":/").length - 1); // https://p.221220.xyz/https://example.com (没有/在最后)
...
<body>
    <h3 class="center">
        I made this project because some extreme annoying network filter software in my school, which is notorious "Goguardian", and now it is open source at <a href="https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/">https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/</a>.
      </h3>
      <br><br><br>
      ...
      <li class="important">How to use this proxy:<br>
        ...
        https://p.221220.xyz/https://the current url/github.com<br>OR<br>https://the current url/https://p.221220.xyz/https://github.com</li>
      </ul>
        ...
        <script>
            function redirectToProxy(event) {
                ...
                const currentOrigin = window.__location____.origin;
    ...
    <h3>
        ...
        <br><br>
        <span>Bypass by proxy: You can buy a domain($1) and setup by yourself: </span><a href="https://p.221220.xyz/https://github.com/1234567Yang/cf-proxy-ex/blob/main/deploy_on_deno_tutorial.md">how to setup a proxy</a><span>. Unless they use white list mode, this can always work.</span>
        <br><br>
        <span>Youtube video unblock: "Thanks" for Russia that they started to invade Ukraine and Google blocked the traffic from Russia, there are a LOT of mirror sites working. You can even <a href="https://p.221220.xyz/https://github.com/iv-org/invidious">setup</a> one by yourself.</span>
    </h3>
    ...
    <head>
        <script>
            function setPassword() {
                try {
                    var cookieDomain = window.__location____.hostname;
                    var password = document.getElementById('password').value;
                    var currentOrigin = window.__location____.origin;
                ...
                //window.__location____.href = currentOrigin + "?" + oneWeekLater.toUTCString();
                location.reload();
            }
        </script>
    </head>
...
async function handleRequest(request) {
  ...
  if (request.url.endsWith("favicon.ico")) {
    return getRedirect("https://p.221220.xyz/https://www.baidu.com/favicon.ico");
  }
  ...
  try {
    ...
    if (!test.startsWith("http")) {
      test = "https://p.221220.xyz/https://" + test;
    }
    var u = new URL(test);
    if (!u.host.includes(".")) {
      throw new Error();
  ...
  catch { //可能是搜素引擎，比如proxy.com/https://p.221220.xyz/https://www.duckduckgo.com/ 转到 proxy.com/?q=key
    var lastVisit;
    if (siteCookie != null && siteCookie != "") {
      lastVisit = getCook(lastVisitProxyCookie, siteCookie);
      console.log(lastVisit);
      if (lastVisit != null && lastVisit != "") {
        //(!lastVisit.startsWith("http"))?"https://p.221220.xyz/https://":"" + 
        //现在的actualUrlStr如果本来不带https://p.221220.xyz/https:// 的话那么现在也不带，因为判断是否带protocol在后面
  ...
  if (!actualUrlStr.startsWith("http") && !actualUrlStr.includes("://")) { //从www.xxx.com转到https://p.221220.xyz/https://www.xxx.com
    //actualUrlStr = "https://p.221220.xyz/https://" + actualUrlStr;
    return getRedirect(thisProxyServerUrlHttps + "https://p.221220.xyz/https://" + actualUrlStr);
  }
  ...
  //check for upper case: proxy.com/https://p.221220.xyz/https://ABCabc.dev
  {
  ...
  const modifiedRequest = new Request(actualUrl, {
    ...
    redirect: "manual"
    //因为有时候会
    //https://p.221220.xyz/https://www.jyshare.com/front-end/61   重定向到
    //https://p.221220.xyz/https://www.jyshare.com/front-end/61/
    //但是相对目录就变了
  });
  ...
  if (response.body) {
    if (contentType && contentType.startsWith("text/")) {
      ...
      if (contentType && (contentType.includes("html") || contentType.includes("javascript"))) {
        bd = bd.replaceAll("window.__location____", "window." + replaceUrlObj);
        bd = bd.replaceAll("document.__location____", "document." + replaceUrlObj);
      }
      ...
      if (contentType && contentType.includes("text/html") && bd.includes("<html")) {
        ...
        //https://p.221220.xyz/https://en.wikipedia.org/wiki/Byte_order_mark
        var hasBom = false;
  ...
  if (contentType && contentType.includes("text/html") && response.status == 200 && bd.includes("<html")) { //如果是HTML再加cookie，因为有些网址会通过不同的链接添加CSS等文件
    ...
    //origin末尾不带/
    //例如：console.log(new URL("https://p.221220.xyz/https://www.baidu.com/w/s?q=2#e"));
    //origin: "https://p.221220.xyz/https://www.baidu.com"
  ...
  const popup = window.open('https://p.221220.xyz/https://different-origin.com'); 
  popup 将会是 null
  ...
  <img src="https://p.221220.xyz/https://other-domain.com/image.jpg"> 
...
// console.log(isPosEmbed("<script src='https://p.221220.xyz/https://www.google.com/'>uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu</script>",2));
// VM195:1 false
// console.log(isPosEmbed("<script src='https://p.221220.xyz/https://www.google.com/'>uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu</script>",10));
// VM207:1 false
// console.log(isPosEmbed("<script src='https://p.221220.xyz/https://www.google.com/'>uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu</script>",50));
// VM222:1 true
